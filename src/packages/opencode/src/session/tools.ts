...
export const resolve = Effect.fn("SessionTools.resolve")(function* (input: {
  ...
  processor: Pick<
    ...
    "message" | "updateToolCall" | "completeToolCall"
  >;
      ...
      execute(args, options) {
        return run.promise(
          Effect.gen(function* () {
            ...
            const toolArgs = before.args;
            const result = yield* item.execute(toolArgs, ctx);
            const output = {
              ...result,
        ...
        return run.promise(
          Effect.gen(function* () {
            ...
            const resources = Object.values(
              yield* mcp.resources(parsed.server),
            );
            const filtered = resources
              .filter(
                (resource) =>
                  !parsed.server || resource.client === parsed.server,
              )
              .toSorted((a, b) =>
            ...
            const content = JSON.stringify(
              { resources: filtered.map(formatMcpResource) },
              null,
              2,
            );
        ...
        return run.promise(
          Effect.gen(function* () {
            ...
            const templates = Object.values(
              yield* mcp.resourceTemplates(parsed.server),
            );
            const filtered = templates
              .filter(
                (template) =>
                  !parsed.server || template.client === parsed.server,
              )
              .toSorted((a, b) =>
                (
                  a.client +
      ...
      execute(args, opts) {
        return run.promise(
          Effect.gen(function* () {
            ...
            if (!content)
              throw new Error(
                `Failed to read MCP resource: ${parsed.server}/${parsed.uri}`,
              );
...
function formatMcpResourceTemplate(
  template: Record<string, unknown> & { client: string },
) {
  const result = Object.fromEntries(
...
function formatMcpResourceContent(
  ...
  const items = (
    Array.isArray(content.contents) ? content.contents : [content.contents]
  ).filter(isRecord);
  const text: string[] = [];
  const attachments: Omit<
    SessionV1.FilePart,
