  ...
  switch (true) {
    case e instanceof DOMException && e.name === "AbortError":
      return new AbortedError(
        { message: timeoutAbortMessage(e.message, ctx.providerID) },
        {
          cause: e,
        },
      ).toObject()
...
function timeoutAbortMessage(message: string, providerID: ProviderV2.ID) {
  if (!/\baborted due to timeout\b/i.test(message)) return message
  return [
    "The operation was aborted due to timeout.",
    `Provider: ${providerID}.`,
    "The model request did not finish before its effective timeout.",
    "Possible causes: a slow or unavailable provider, a stalled network connection, or a request timeout that is too short.",
    "Try again. If it recurs, check the provider and network connection, then increase provider.options.timeout in opencode.json (milliseconds) or set it to false to disable the timeout.",
  ].join("\n")
}
