...
export function policy(opts: {
  ...
  decide?: (
    input: { error: Err; attempt: number },
    output: { retry?: Retryable | false; delay?: number },
  ) => Effect.Effect<void>
}) {
  return Schedule.fromStepWithMetadata(
    Effect.succeed((meta: Schedule.InputMetadata<unknown>) => {
      ...
      return Effect.gen(function* () {
        const output: { retry?: Retryable | false; delay?: number } = {}
        if (opts.decide) yield* opts.decide({ error, attempt: meta.attempt }, output)
        const retry = output.retry === undefined ? retryable(error, opts.provider) : output.retry || undefined
        if (!retry) return yield* Effect.fail(Cause.done(meta.attempt))
        const wait =
          typeof output.delay === "number" && output.delay >= 0
            ? cap(output.delay)
