...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const create = Effect.fn("SessionProcessor.create")(function* (
      input: Input,
    ) {
      ...
      const settleToolCall = Effect.fn("SessionProcessor.settleToolCall")(
      ...
      const readToolCall = Effect.fn("SessionProcessor.readToolCall")(
        function* (toolCallID: string) {
      ...
      const failToolCall = Effect.fn("SessionProcessor.failToolCall")(
        function* (toolCallID: string, error: unknown) {
          const match = yield* readToolCall(toolCallID);
      ...
      const finishReasoning = Effect.fn("SessionProcessor.finishReasoning")(
      ...
      const ensureToolCall = Effect.fn("SessionProcessor.ensureToolCall")(
        function* (input: {
          id: string;
          name: string;
          ...
          if (existing) {
            if (
              ...
              existing.part.metadata?.providerExecuted
            )
              return existing;
      ...
      const toolResultOutput = (
          ...
          return {
            title:
              typeof value.result.value.title === "string"
                ...
                : value.name,
            metadata: isRecord(value.result.value.metadata)
        ...
        return {
          title: value.name,
          metadata:
            value.result.type === "json" && isRecord(value.result.value)
              ? value.result.value
              : {},
          output:
            ...
            const normalized = yield* Effect.forEach(
              ...
              (attachment) =>
                attachment.mime.startsWith("image/")
                  ...
                  : Effect.succeed(
                      Exit.succeed<SessionV1.FilePart>(attachment),
                    ),
            );
      ...
      const process = Effect.fn("SessionProcessor.process")(function* (
        ...
        ctx.shouldBreak =
          (yield* config.get()).experimental?.continue_loop_on_deny !== true;
...
function interruptedToolError(
  error: SessionV1.Assistant["error"],
  tool: string,
) {
  const message = errorMessage(error);
  if (!TIMEOUT_ABORT_PATTERN.test(message)) return "Tool execution aborted";
  return [
    "Tool execution was interrupted because the model request timed out.",
    `Tool: ${tool}.`,
    "The tool may have started, but OpenCode stopped waiting when the model request exceeded its timeout.",
    "The model retry policy will wait 2 seconds before retrying. For long shell commands, use a larger timeout value.",
  ].join("\n");
}
